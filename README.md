...
`docker run -d -p <host_port>:<container_port> nasagibs/onearth:1.3.5 apachectl -D FOREGROUND`

So to run a Docker container that's accessible via port 8080 on the host machine, run:

`docker run -d -p 8080:8080 nasagibs/onearth:1.3.5 apachectl -D FOREGROUND`
